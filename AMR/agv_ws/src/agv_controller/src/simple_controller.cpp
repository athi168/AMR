#include "agv_controller/simple_controller.hpp"
...
SimpleController::SimpleController(const std::string & name): Node(name), left_wheel_prev_pos_(0.0), right_wheel_prev_pos_(0.0), x_(0.0), y_(0.0), theta_(0.0)
{
    ...
    declare_parameter("wheel_separation", 0.402);
    ...
    RCLCPP_INFO_STREAM(get_logger(), "Using gear_ratio " << gear_ratio_);

    prev_time_ = get_clock()->now();

    wheel_cmd_pub_ = create_publisher<std_msgs::msg::Float64MultiArray>("/simple_velocity_controller/commands", 10);
    vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>("/agv_controller/cmd_vel", 10, 
    ...
    odom_pub_ = create_publisher<nav_msgs::msg::Odometry>("/agv_controller/odom", 10);
...
void SimpleController::velCallback(const geometry_msgs::msg::TwistStamped & msg)
{
    ...
    wheel_speed = wheel_speed*gear_ratio_;
